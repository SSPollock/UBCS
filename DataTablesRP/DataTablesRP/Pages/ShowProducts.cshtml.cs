...
using DataTablesRP.Models;
using DataTablesRP.Models.ViewModels;
        ...
        public IActionResult OnPostLoadData()
        {
            try
            {
                ...
                {              
                    ...
                    PList = (PList.AsQueryable<Products>()).OrderBy<Products>(sortColumn + " " + sortColumnDirection).AsEnumerable<Products>();                 
                }                 
                // search has value                 
                if (!string.IsNullOrEmpty(searchValue))                
                {                     
                    PList = PList.Where(m => 
                    (m.ProductName.ToUpper().IndexOf(searchValue.ToUpper())>=0) || (m.UnitPrice.ToString() == searchValue)).ToList<Products>();
                } 
